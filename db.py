...
def store_transport_data(session_id, transport_mode, distance):
    ...
    with sqlite3.connect('data/emissions.db') as conn:
        ...
        conn.commit()  
        print(f"Inserted transport data: {session_id}, {transport_mode}, {distance}")  # Debugging
...
def store_food_data(session_id, dietary_pattern, food_choices):
    ...
    with sqlite3.connect('data/emissions.db') as conn:
        ...
        conn.commit()
        print(f"Inserted food choices: {food_choices} for session: {session_id}")  # Debugging
